...
def check_ttl(redis_conn, no_ttl_file, dbindex, scannum_thensleep):
    ...
    # 打印扫描db开始时间
    print( f"startTime of db[{dbindex}] is ：{time.strftime( '%Y-%m-%d %H:%M:%S', time.localtime() )}" )
    process_bar = ShowProcess( keys_num )
    with open( no_ttl_file, 'a' ) as f:

        for key in redis_conn.scan_iter( count=1000 ):
            key_write=key.decode()
            key_ttl=redis_conn.ttl(key)
            res=process_bar.show_process()
            print('\r',"目前进展:",res,"已经扫描的key数量:",)

            #print(res)
            if key_ttl == -1:
                no_ttl_num += 1
                if no_ttl_num < 100000:
                    f.write( key_write+'\n')
            else:
                continue
            scan_num += 1
    ...
    process_bar.close()
    # 打印扫db描结束时间
    print( "endTime of db[{index}] is ：{end_time}".format( index=dbindex, end_time=time.strftime( "%Y-%m-%d %H:%M:%S",
                                                                                                  time.localtime() ) ) )
    ...
    for index in db_list:
        try:
            pool = redis.ConnectionPool( host="192.168.37.8", port=6379, db=index,
                                         password=123 )  # 20201205，支持传入Redis密码
